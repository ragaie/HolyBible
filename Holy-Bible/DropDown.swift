    ...
    func showSelection(_ sender : UIButton)  {
        ...
        if showDrop == true {
            ...
            if tableData == nil {
                ...
                var  originalPoint = frame.origin
                var layer = self.layer
                var point = originalPoint
                while ((layer.superlayer) != nil)
                {
                    point = layer.convert(point, to: layer.superlayer)
                ...
                tableData  = UITableView.init(frame: CGRect.init(x: frame.origin.x + 5 , y: frame.origin.y + frame.height, width: frame.width - 10, height:  CGFloat(30 * dataSourceItem.count)))
          ...
          // get owner that containe this view
            ...
            //layer.addSublayer(tableData.layer)
            showDrop = true
        }
